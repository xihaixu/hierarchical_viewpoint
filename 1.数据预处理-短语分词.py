...
alphaPattern = "[^a-zA-Z0-9____]"  # 要注意，这样会把标点符号去掉。可以把要保留的标点符号直接加载列表里面，例如;表示保留分号
...
def changetoRakeText(textdata):
    ...
    for text in textdata:
        if len(str(text))>0 and str(text)!='nan':
            ...
            s = text
            ##判断关键词是否是多个单词组合，是的话则将原文中关键词组中的" "替换为"____"，方面区分
            for phrase in phrases:
                if " " in phrase:
                    s = s.replace(phrase, phrase.replace(" ", "____"))
            processedText.append(s)
        else:
